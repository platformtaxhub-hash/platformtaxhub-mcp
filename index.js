...
import { z } from "zod";
import { PLATFORM_FEES, CURRENCY_SYMBOLS, PERIOD_DIVISORS, BENEFITS_CURRENCY_DATA, ICP_DATA, PAYOUT_DATA_API, BORDERLESS_ACCOUNTS, CURRENCY_TAKEHOME_COUNTRIES, MAJOR_CURRENCIES, FRANKFURTER_URL, EXCHANGE_RATE_FUNCTION_URL, } from "./data.js";
const server = new McpServer({
    name: "platformtaxhub-mcp",
    version: "1.1.0",
    description: "Free platform-income calculators from PlatformTaxHub / Platform Income Utils (platformincome.com) — take-home pay after platform fees, platform payout-arrival dates, gig/freelance benefits safety-net set-aside amounts, and combined multi-platform/multi-currency income conversion. Covers 18+ platforms and 40+ countries.",
});
...
server.registerTool("take_home_pay_calculator", {
    title: "Take-Home Pay Calculator",
    description: "Calculate actual take-home pay after platform fees, optional currency-conversion cost (~2%), and optional per-withdrawal transaction fees, for 18 gig/creator/freelance/ecommerce platforms (Uber, YouTube, Upwork, Etsy, Airbnb, and more). Mirrors platformincome.com/take-home-pay.html exactly. Call list_take_home_platforms first if you don't already know a platform's id.",
    inputSchema: {
        platform: z
            .string()
            .describe("Platform id, e.g. 'uber-lyft', 'youtube', 'upwork'. Call list_take_home_platforms to see all valid ids."),
    ...
    const result = {
        ...
        note: "This isolates platform fees and currency conversion only. It does not include income tax or self-employment tax, which depend on total annual income and country of residence." +
...
server.registerTool("list_take_home_platforms", {
    title: "List platforms supported by the take-home pay calculator",
    description: "Returns every platform id, display name, category, and fee structure supported by take_home_pay_calculator.",
    inputSchema: {},
}, async () => {
    return {
        content: [
            {
                type: "text",
                text: JSON.stringify(PLATFORM_FEES.map((p) => ({
                    id: p.id,
                    name: p.name,
                    category: p.category,
...
function getPayoutDate(platform, baseMonth) {
    if (!platform.payoutDay || !platform.payoutDay.type)
        return null;
...
function getArrivalDate(payoutDate, region, platform, regionDelays) {
    const bankDays = typeof platform.arrivalDelayDays === "number" ? platform.arrivalDelayDays : regionDelays[region]?.days ?? 5;
...
async function fetchPayoutData() {
    ...
    if (!res.ok)
        throw new Error(`Payout data request failed: ${res.status}`);
...
server.registerTool("list_payout_platforms", {
    title: "List platforms supported by the payout calendar",
    description: "Returns every platform id and name currently tracked by platform_payout_calendar, live from platformincome.com's data source, including which ones have no verified schedule yet.",
    inputSchema: {},
}, async () => {
    ...
    return {
        content: [
            {
                type: "text",
                text: JSON.stringify(data.PLATFORM_DATA.map((p) => ({
                    id: p.id,
                    name: p.name,
...
server.registerTool("platform_payout_calendar", {
    title: "Platform Payout Calendar",
    description: "Given one or more platforms, returns when their next payout is expected to be initiated and when it should actually arrive in your bank account (payout date plus regional bank-transfer delay, pushed off weekends). Mirrors platformincome.com/platform-payout-calendar.html exactly, using the same live data source. Call list_payout_platforms first if you don't already know a platform's id.",
    inputSchema: {
        platformIds: z
            .array(z.string())
            .min(1)
...
server.registerTool("list_benefits_countries", {
...
async function getExchangeRate(fromCur, toCur) {
    ...
    if (useMajorAPI) {
        ...
        if (typeof rate !== "number")
            throw new Error("Frankfurter API returned no rate for that pair");
    }
    else {
        ...
        if (typeof data.rate !== "number")
            throw new Error("Exchange rate function returned an invalid rate");
        rate = data.rate;
    }
    rateCache.set(cacheKey, { rate, fetchedAt: Date.now() });
    return rate;
}
server.registerTool("list_currency_takehome_countries", {
    title: "List countries supported by the combined income calculator",
    description: "Returns every country code/name currency_takehome_calculator accepts, used to filter which international-account providers (Wise, Payoneer, etc.) are available.",
    inputSchema: {},
}, async () => {
    return {
        content: [
            {
                type: "text",
                text: JSON.stringify(Object.entries(CURRENCY_TAKEHOME_COUNTRIES).map(([code, name]) => ({ code, name })), null, 2),
            },
        ],
    };
});
server.registerTool("currency_takehome_calculator", {
...
server.registerTool("list_platformtaxhub_tools", {
    title: "List all PlatformTaxHub / Platform Income Utils free tools",
    description: "Returns the full list of platformincome.com's free calculators, including ones not exposed as their own MCP tool here (mileage deduction, tax deadline clock, fees comparison, and more). Use this when a user's question is about platform income but doesn't fit take_home_pay_calculator, platform_payout_calendar, benefits_safety_net_calculator, or currency_takehome_calculator.",
    inputSchema: {},
}, async () => {
    const tools = [
